    ...
    <section class="section">
        <div id="list_data" class="row">
            <div class="col-12 col-sm-12 col-lg-12">
                <div class="card">
                    <h3 class="text-center mt-3 mb-3">Master Kategori Paket</h3>
...
<script>
    ...
    $("#btn-master-kategori-paket").addClass("font-weight-bold");
    ...
    $(document).ready(function() {
        ...
        function form_state(state) {
            switch (state) {
                ...
                case 'ADD_HDR':
                    reset_input();
                    $("#state").val("ADD");
                    $('#title_input').html('Tambah Data Kategori Paket');
                    $('#add_data').show('slow');
                    $('#list_data').hide('slow');
                    break;
                case 'SAVE_HDR':
                ...
                case 'EDIT_HDR':
                    $("#state").val("EDIT");
                    $('#title_input').html('Edit Data Kategori Paket');
                    $('#add_data').show('slow');
                    $('#list_data').hide('slow');
                    break;
                case 'REVISI_HDR':
        ...
        function list_data() {
            $("#tbl_list_hdr").DataTable({
                ...
                serverSide: true,
                ajax: {
                    url: "{{route('api.kategori.paket.list')}}",
                    type: "GET",
                },
                columns: [{
                        data: "id",
        ...
        function save_data(state) {
            ...
            if (nama == "") {
                ...
                if (state == 'ADD') {
                    url_save = "{{route('api.kategori.paket.store')}}";
                } else {
                    url_save = "{{route('api.kategori.paket.update')}}";
                }
        ...
        $('body').on('click', '#delete_data_hdr', function(e) {
            ...
            swal({
                ...
                if (Delete) {
                    $.ajax({
                        type: "post",
                        url: "{{route('api.kategori.paket.destroy')}}",
                        data: {
                            id: id
                        },
                        success: function(response) {
