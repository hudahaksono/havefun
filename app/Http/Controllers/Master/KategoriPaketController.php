...
class KategoriPaketController extends Controller
{
    public function index()
    {
        ...
        $user_baru = DB::table('mst_users')
                        ...
                        ->where('created_at','>=',$date)
                        ->get();
        return view('office.kategori.master-kategori-paket', compact('order_baru','pembayaran','user_baru'));
    }
    ...
    public function list_data_hdr(Request $request)
    {
        ...
        $totalData = DB::table('tmst_kategori_paket')
        ...
        if (empty($request->input('search.value'))) {
            $posts = DB::table('tmst_kategori_paket')
                ->offset($start)
                ->limit($limit)
                ->where('status_hapus', '=', 0)
                ->orderBy('nama')
            ...
            $totalFiltered = DB::table('tmst_kategori_paket')
                // ->where([['status_hapus', '=', 0], ['kode', 'LIKE', "%{$search}%"]])
                ->Where([['status_hapus', '=', 0], ['nama', 'LIKE', "%{$search}%"]])
                ->count();
        }
    ...
    {
        $grup_kategori    = KategoriPaketModels::where('nama', $request->nama)->first();
        ...
        if ($grup_kategori === NULL) {            
            ...
            $data_save_hdr = KategoriPaketModels::create($data_hdr);
    ...
    public function update(Request $request)
    {
        ...
        KategoriPaketModels::where('id', $request->sysid)->update($data);
    ...
    public function destroy(Request $request)
    {
        ...
        KategoriPaketModels::where('id', $request->id)->update($data);
